...
export class RecoverPasswordComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.params.subscribe(({ uid, token }) => {
      this.uid = uid;
      this.token = token;
      sessionStorage.setItem('net_token', token);
  ...
  ChangePassword(){
    if (this.new_password1 === this.new_password2) {
      ...
      this.authSvc.ConfirmNewPassword(body)
          .subscribe({
            error:(err:any) => {
              ...
              Swal.fire('Ooops', 'La contraseña no cumple con los criterios de seguridad, intente', 'error');
            },
            next:(resp:any) => {
              console.log(resp);
      ...
      Swal.fire('Ooops', 'Las contraseñas no corresponden', 'error');
      this.new_password1 = '';
      this.new_password2 = '';
    }
  }
